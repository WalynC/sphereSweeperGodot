...
func exit_screen():
	...
	if (upControl):tween.tween_property(upControl, "position", Vector2(0,-upControl.get_parent_control().size.y),.5)
	return tween

func enter_screen():
	var tween = create_tween()
	tween.tween_property(control, "position", Vector2(0,0),.5)
	if (upControl):tween.tween_property(upControl, "position", Vector2(0,0),.5)
